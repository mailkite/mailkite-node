...
export class MailKite {
  ...
  // --- Messages & deliveries ------------------------------------------------
  listMessages(before, limit, search) {
    const qs = [];
    if (before != null) qs.push(`before=${encodeURIComponent(before)}`);
    if (limit != null) qs.push(`limit=${encodeURIComponent(limit)}`);
    if (search != null) qs.push(`search=${encodeURIComponent(search)}`);
    const suffix = qs.length ? `?${qs.join("&")}` : "";
    return this.request("GET", `/api/messages${suffix}`);
  }
  getMessage(id) {
  ...
  // --- Docs -----------------------------------------------------------------
  // Semantic search over the MailKite docs. Public — no auth required (the auth
  // header is sent anyway, which is harmless). Returns { query, matches[] }.
  semanticSearch(query) {
    return this.request("GET", `/v1/docs/search?query=${encodeURIComponent(query)}`);
  }
  ...
  listListContacts(id, before, limit) {
    const qs = [];
    if (before != null) qs.push(`before=${encodeURIComponent(before)}`);
    if (limit != null) qs.push(`limit=${encodeURIComponent(limit)}`);
    const suffix = qs.length ? `?${qs.join("&")}` : "";
    return this.request("GET", `/api/lists/${encodeURIComponent(id)}/contacts${suffix}`);
  }
